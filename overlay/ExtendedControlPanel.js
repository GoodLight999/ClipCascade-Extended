...
export default function ExtendedControlPanel({ NativeBridgeModule, notifee }) {
  ...
  const [dialog, setDialog] = useState({ visible: false, title: '', body: '', copy: '' });

  const show = (title, body, copy = body) => {
    setDialog({ visible: true, title, body: String(body), copy: String(copy) });
  };
  ...
  const showStatus = () =>
    run(async () => {
      const status = JSON.parse(await NativeBridgeModule.getReliabilityStatus());
      const lines = [
        ...
        `P2P candidates/compatible/incompatible: ${status.p2pCandidatePeers || 0}/${status.p2pCompatiblePeers || 0}/${status.p2pIncompatiblePeers || 0}`,
  ...
  const runAutoDebug = () =>
    run(async () => {
      ...
      const probe = NativeBridgeModule.runNativeAutoDebug
        ...
        : { clipboardRead: null, reason: 'native probe unavailable' };
  ...
  const runShizuku = () =>
    run(async () => {
      try {
        ...
        const status = JSON.parse(await NativeBridgeModule.getShizukuStatus());
        show(
          'Shizuku',
  ...
  const button = (label, onPress, backgroundColor = '#2457a6') => (
    <TouchableOpacity
  ...
  return (
    <View style={styles.card}>
      ...
      {button(text.accessibility, () => NativeBridgeModule.openAccessibilitySettings(), '#176b3a')}
      {button(text.overlay, () => NativeBridgeModule.openOverlaySettings(), '#176b3a')}
      {button(
        text.shizukuOpen,
        () =>
          run(async () => {
      ...
      {button(text.battery, () => notifee.openBatteryOptimizationSettings(), '#48515a')}
      {button(text.power, () => notifee.openPowerManagerSettings(), '#48515a')}
      ...
      >
        <View style={styles.backdrop}>
          <View style={styles.dialog}>
            ...
            <ScrollView style={styles.dialogScroll} contentContainerStyle={styles.dialogBodyContainer}>
            ...
            <View style={styles.dialogActions}>
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  setDialog(prev => ({ ...prev, title: text.copied }));
                }}
              >
